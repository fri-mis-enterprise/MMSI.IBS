...
- **DELETED filter button restyled** — ServiceRequest & DispatchTicket Index DELETED buttons now use the shared `modern-btn-error` (red text/border, clearly visible on light) instead of the inconsistent inline `--on-surface-variant`/`--error` styles, and get a red `.active` fill instead of primary blue when toggled (keeps `modern-btn-secondary` so existing filter/active JS works). (`ServiceRequest/Index.cshtml`, `DispatchTicket/Index.cshtml`)
