...
- **Razor verification guidance corrected** — The Web SDK already compiles .cshtml on build/publish. A deliberately invalid temporary view failed the build with CS0117, confirming enforcement. AGENTS.md now requires builds after Razor edits, with browser checks for HTML/JavaScript behavior.
- **Validation** — Final build passed with 0 warnings/errors; temporary checks outside the solution exercised import rounding, Service Request restore/post/attachment guards and audit ordering, bank-account audit ordering, SuperAdmin case-insensitive search and status rendering. Authenticated browser verification remains pending.
...
- **SuperAdmin data browser no longer loads whole tables** — previously `GetDataAsync` called `Repository.GetAllAsync` (→ `ToListAsync`, every row) then reflected over all rows in memory to filter/sort/skip/take. Added `IRepository<T>/Repository<T>.GetPagedAsync(filter, orderBy, orderDir, skip, take)` (same SQL-server pattern as the existing `GetPagedJobOrdersAsync` precedent, `AsNoTracking` + `EF.Property` dynamic order) and rewrote `GetDataAsync` to push per-table search/sort/paging down to SQL, mapping only the current page. Removed dead in-memory `QueryData`/`ApplySort` reflection helpers. (`IRepository.cs`, `Repository.cs`, `SuperAdminService.cs`)
- **DELETED filter button restyled** — `ServiceRequest/Index.cshtml` & `DispatchTicket/Index.cshtml` DELETED buttons now use the shared `modern-btn-error` (red text/border, clearly visible on light) instead of the inconsistent inline `--on-surface-variant`/`--error` style, and get a red `.active` fill instead of primary blue when toggled (keeps `modern-btn-secondary` so existing filter/active JS works). (`ServiceRequest/Index.cshtml`, `DispatchTicket/Index.cshtml`)
...
- **Dispatch Ticket hours floor raised 0.5h → 1h** — `CreateDispatchTicketAsync` and `UpdateDispatchTicketAsync` now use `Math.Max(duration, 1m)` so any sub-1-hour ticket bills a minimum of 1 hour (fractional durations above 1h unchanged). Same 1h floor applied to legacy `ServiceRequestController` create/edit. (`IBS.Services/DispatchTicketService.cs`, `IBSWeb/Areas/User/Controllers/ServiceRequestController.cs`)
...
- **MCP `audit_views` tool** — Batch-scans `.cshtml` views (Razor/C# compilation runs at build; these checks cover additional view conventions) against ARCHITECTURE.md §4.4. Rules: V1 (Index pages must not declare `@model`), V2 (Create/Edit must use a `*ViewModel`, not a domain entity), V3 (full pages must use `.modern-layout`), V4 (script blocks using `fetch()` must check `.success` or have `.catch`/`.then` error path), V5 (balanced structural tags: div/form/table/thead/tbody/tr/select/main/section, after stripping script/style/comment blocks), V6 (`.success`/`.error`/`.message` read on a variable that is neither declared, a known global, nor a function param → runtime ReferenceError), V7 (`@model` declared but never referenced via `Model.*` or `asp-for`). Accepts optional `folder` to narrow. Baseline: 57 findings (32 V2, 12 V7, 9 V1, 4 V3; V4/V5/V6 clean). (`mcp-server/src/tools/audit-views.ts`, `mcp-server/src/utils/formatter.ts`)

### Changed
- **Master-file Create/Edit screens converted to ViewModels** — Created 5 new `XViewModel : XEntity` classes (`PaymentTermsViewModel`, `SupplierViewModel`, `CustomerViewModel`, `EmployeeViewModel`, `BankAccountViewModel` in `IBS.Models/MasterFile/`; completes the 14-VM set started with the MSAP VMs in `IBS.Models/MSAP/ViewModels/`). All 14 controllers now build a VM in GET (Create/Edit) and bind the VM in POST (services keep receiving the entity via inheritance). Swapped `@model` on all 28 Create/Edit views to the VM. Fixed `<main>` tag imbalance in `MsapImport/Index.cshtml`. View audit dropped 38 → 9 findings, all remaining are documented deviations (Billing/UserAccess) or verified false positives (Home/Docs layout, PostedPeriod/_Timeline `Model` usage). Documented the Billing + UserAccess intentional deviations in `Docs/ARCHITECTURE.md` §5.2 (V4). Build clean (0 warnings/errors).
...
- **"Edit Tariff" action on Job Order Details** — Added to the "For Approval" dropdown alongside Approve/Disapprove. (`JobOrder/Details.cshtml`)

### Changed
- **Navbar hides MSAP workflow links the user can't access** — Each core MSAP menu link (Job Orders, Dispatch Tickets, Billing, Collection, MMSI Reports) is now wrapped in an `@if` mirroring its Index action's `RequireAnyAccess` gate, so the menu shows only modules the user has at least one matching procedure for (MSAP Import was already gated). Added a single `ModernHasAnyAccess(params ProcedureEnum[])` helper in `_Navbar.cshtml` and simplified `ModernHasMsapAccess` to reuse it. MSAP References menu intentionally left as-is for now. (`IBSWeb/Views/Shared/_Navbar.cshtml`)
- **Access denial is transport-aware again** — `RequireAccessBaseAttribute` (new base for both attributes) returns the `{ success = false, message }` JSON envelope for AJAX/JSON requests and redirects full-page navigations back to the local referer with `TempData["error"]` (rendered as a ModernAlert by `_Notification.cshtml`), falling back to `Home/Index` when there's no same-origin referer (e.g. a directly typed URL). Fixes raw JSON showing in the browser when navigating directly to an unauthorized page and no longer drops the user at Home on every denial. Also gated the Create entry buttons that were visible despite lacking the permission: JobOrder Details "ADD TICKET" (`CanCreateDispatchTicket`), Billing Index "CREATE BILLING" (`CanPost`), JobOrder Index "CREATE JOB ORDER" (`CanCreate`), ServiceRequest Index "CREATE REQUEST" (`CanEdit`). (`IBS.Services/Attributes/RequireAccessAttribute.cs`, `JobOrderController.cs`, `JobOrder/{Index,Details}.cshtml`, `Billing/Index.cshtml`, `ServiceRequest/Index.cshtml`)
- **Permission-aware action visibility on MSAP workflow pages** — Action buttons/dropdown items are now hidden when the user lacks the underlying procedure (previously only status-gated). Controllers compute `ViewBag.Can*` flags via `IAccessControlService` (following Billing's existing `CanReverse` pattern): DispatchTicket Index (`CanSetTariff`/`CanEditTicket`/`CanApproveTariff`/`CanDeleteTicket`) + Preview (approve/disapprove gated by `CanApproveTariff`, revoke by `CanEditTicket`); JobOrder Index (`CanEdit`) + Details (header Edit, batch toolbar + checkboxes, ticket dropdown Set/Edit Tariff, Approve/Disapprove, Edit/Delete Ticket, Post/Edit Request); Billing Index (`CanPost`/`CanEdit`/`CanDelete`); ServiceRequest Index (Edit/Delete/Restore by `CanEdit`). Also wired up the previously-undefined `confirmApprove()` in DispatchTicket Index (dropdown referenced it, JS function was missing). (`DispatchTicketController.cs`, `JobOrderController.cs`, `BillingController.cs`, `ServiceRequestController.cs`, `DispatchTicket/{Index,Preview}.cshtml`, `JobOrder/{Index,Details}.cshtml`, `Billing/Index.cshtml`, `ServiceRequest/Index.cshtml`, `JobOrderControllerTests.cs`)
- **Removed dead access-control API** — Dropped `HasAllAccessAsync`, `GetAccessMapAsync`, and 10 unused `AccessControlExtensions` (`HasJobOrderAccessAsync`, `HasDispatchTicketAccessAsync`, `HasBillingAccessAsync`, `HasCollectionAccessAsync`, `HasTariffAccessAsync`, `HasMsapAccessAsync`, `HasTreasuryAccessAsync`, `HasDisbursementAccessAsync`, `HasGeneralLedgerReportAccessAsync`, `HasInventoryReportAccessAsync`) — all had zero callers (verified across `.cs` + `.cshtml`). `IAccessControlService` is now just `HasAccessAsync` + `HasAnyAccessAsync`. (`IBS.Services/AccessControl/AccessControlService.cs`)
- **RequireAccess/RequireAnyAccess simplified to a single JSON denial** — Both attributes now always return `{ success = false, message }` at HTTP 200 (previously 403 JSON for AJAX, TempData["Denied"] + redirect for non-AJAX). This matches the uniform envelope every MSAP action already returns and makes `fetch`, jQuery, and DataTables all parse denials identically (403 previously routed to jQuery's `error` callback, so `ModernAlert.error` never fired). Dropped the redirectController/redirectAction/redirectArea constructor params and the unused 5-arg `RequireAnyAccess` constructor (15 call sites in DispatchTicketController updated). Removed the now-dead `TempData["Denied"]` branch from `_Notification.cshtml`. Trade-off: non-AJAX navigation to a denied action renders the JSON envelope instead of redirecting — global `FallbackPolicy` in Program.cs still handles unauthenticated users. (`IBS.Services/Attributes/RequireAccessAttribute.cs`, `DispatchTicketController.cs`, `_Notification.cshtml`)
- **tutorial.js tour engine fixes** — Container steps now advance on `change/input` of contained inputs instead of raw `click` (which blocked native date/time pickers); 250ms debounce guard prevents double-advance when a radio fires both `input` and `change`; `required:false` in step config overrides HTML `required` gating of Next. Removed `sticky top-24` from the Timeline card in DispatchTicket/Create — sticky creates a stacking context that traps the tour's interactive z-index below the backdrop. (`wwwroot/js/tutorial.js`, `DispatchTicket/Create.cshtml`)
- **AJAX authorization returns JSON** — `RequireAccessAttribute` now returns 403 JSON when the request carries `X-Requested-With: XMLHttpRequest` (previously only `Content-Type: application/json`), fixing `SyntaxError: Unexpected token '<'` on permission-denied form-encoded fetches. Added the XHR header to fetches in JobOrder/Details (ApproveTariff, DisapproveTariff, Delete), DispatchTicket/Index (Delete, Restore), ServiceRequest/Index (Delete, Restore). (`IBS.Services/Attributes/RequireAccessAttribute.cs`)

## [2026-08-01]
### Changed
- **Simplified Dispatch Ticket workflow: split SR enum + merged `Pending`** — Created `ServiceRequestStatus` (`Draft`, `Requested`, `ServiceRequestDeleted`) in `SD.cs` and repointed all SR code to it (`ServiceRequestController` create/edit/delete/restore/list, `ServiceRequest/Index.cshtml`, dashboard SR-for-posting count, and SR-exclusion filters in `JobOrderRepository`, `DispatchTicketRepository`, `JobOrderService`, `BillingService`). Slimmed `DispatchTicketStatus` to `ForTariff`, `ForApproval`, `Disapproved`, `ForBilling`, `Billed`, `Deleted`. Merged `Pending` into `ForTariff` everywhere (create/restore always set `ForTariff`; SetTariff batch, delete guard, unassign-tugboat, billing-unready set, dashboard KPI, DT Index dropdown conditions, JobOrder Details batch-tariff JS). No data migration needed — no `Draft`/`Pending`/`ServiceRequestDeleted` rows existed (only 1 `Requested`, 4 `Deleted`). Updated `Docs/manual/02-dispatch-ticket.md`, `Docs/ARCHITECTURE.md`, and `Docs/WORKFLOW-DESIGN-REVIEW.md`. (`SD.cs`, `ServiceRequestController.cs`, `DispatchTicketController.cs`, `HomeController.cs`, `DispatchTicketService.cs`, `JobOrderService.cs`, `BillingService.cs`, `JobOrderRepository.cs`, `DispatchTicketRepository.cs`, `modern-dashboard.js`, `Home/Index.cshtml`, `ServiceRequest/Index.cshtml`, `DispatchTicket/Index.cshtml`, `JobOrder/Details.cshtml`, `Data/Index.cshtml`)
- **Removed unreachable `Cancelled` status from Dispatch Ticket workflow** — Dropped `Cancelled` from `DispatchTicketStatus` constant + `All` array (`SD.cs`), deleted orphaned `ServiceRequestController.PostSelected`/`CancelSelected` endpoints, removed `Cancelled` from `DispatchTicketController.ChangeStatus` valid targets and terminal-state guard, dashboard KPI count (`HomeController` + `Home/Index.cshtml` + `modern-dashboard.js`), SR edit guard, SR Index CANCELLED filter button/JS, JobOrder billable-search + auto-close status sets, and badge/status maps in `JobOrder/Details.cshtml` + SuperAdmin `Data/Index.cshtml`. Also removed dead `confirmCancel()` JS in JobOrder Details (referenced a non-existent `#cancelForm`). No data migration needed — DB had zero `Cancelled` rows. `VesselScheduleStatus.Cancelled` untouched. (`SD.cs`, `ServiceRequestController.cs`, `DispatchTicketController.cs`, `HomeController.cs`, `JobOrderRepository.cs`, `JobOrderService.cs`, `modern-dashboard.js`, `Home/Index.cshtml`, `ServiceRequest/Index.cshtml`, `JobOrder/Details.cshtml`, `Data/Index.cshtml`)
- **Docs audit: corrected workflow mermaid charts** — `Docs/manual/02-dispatch-ticket.md` now reflects the real flow the UI allows: Service Request create → `Draft`/`Requested`; SR post `Requested → For Tariff` (was wrongly `Requested → Pending`); `Pending`/`For Tariff` set tariff → `For Approval`; `Disapproved → For Approval` via edit tariff (was wrongly `→ For Tariff`); added `ForBilling ⇄ ForApproval` (revoke approval), `Billed ⇄ ForBilling` (reverse billing), and `Deleted`/restore states. `Docs/manual/03-billing.md` chart/table fixed to drop removed `Paid` status and show `ForCollection → Collected` + reverse. `Docs/ARCHITECTURE.md` state machine updated to match. (`Docs/manual/02-dispatch-ticket.md`, `Docs/manual/03-billing.md`, `Docs/ARCHITECTURE.md`)
- **Separated Service Request from Dispatch Ticket flow in docs** — Service Request now documented as its own short lifecycle (`Draft ⇄ Requested → For Tariff`, `Service Request Deleted`), independent of the Dispatch Ticket flow. Dropped `Cancelled` from both charts. Created `Docs/WORKFLOW-DESIGN-REVIEW.md` capturing the proposed code cleanup (separate SR status enum, remove `Cancelled`, remove orphaned endpoints); `Cancelled` removal and orphaned-endpoint deletion are now done, SR enum split + service-level guards remain TODO. (`Docs/manual/02-dispatch-ticket.md`, `Docs/manual/05-service-request.md`, `Docs/ARCHITECTURE.md`, `Docs/WORKFLOW-DESIGN-REVIEW.md`)
...
- **Cancelled tickets excluded from billable JO search** — `SearchBillableJobOrdersAsync` didn't include `Cancelled` in the `.All()` condition, so JOs with cancelled tickets never appeared in the billing dropdown. (`JobOrderRepository.cs:85`)
- **JO modifications not restricted when billing exists** — Details view now hides Edit Details, ADD TICKET, batch toolbar, and all ticket action dropdown items when the JO has any billing. A lock notice is displayed instead. (`JobOrderController.cs:139`, `Details.cshtml`)
