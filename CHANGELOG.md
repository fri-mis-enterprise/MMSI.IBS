...
- **Separated Service Request from Dispatch Ticket flow in docs** — Service Request now documented as its own short lifecycle (`Draft ⇄ Requested → For Tariff`, `Service Request Deleted`), independent of the Dispatch Ticket flow. Dropped `Cancelled` from both charts — it is unreachable from any UI surface (`CancelSelected`/`PostSelected` controller actions have no view callers; `ChangeStatus` is only used by Preview's Revoke Approval). Created `Docs/WORKFLOW-DESIGN-REVIEW.md` capturing the proposed code cleanup (separate SR status enum, remove `Cancelled`, remove orphaned endpoints) as a TODO. (`Docs/manual/02-dispatch-ticket.md`, `Docs/manual/05-service-request.md`, `Docs/ARCHITECTURE.md`, `Docs/WORKFLOW-DESIGN-REVIEW.md`)
