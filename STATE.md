...
### Current Focus
- MCP `audit_views` tool added (batch-scans .cshtml vs ARCHITECTURE §4.4: V1 no @model on Index, V2 *ViewModel on Create/Edit, V3 modern-layout, V4 fetch error handling, V5 tag balance, V6 undeclared var reads, V7 unused @model). Baseline 2026-08-04: 57 findings (32 V2, 12 V7, 9 V1, 4 V3; V4/V5/V6 clean). Rebuild: `npm run build` in mcp-server. Restart opencode to load the new tool. Next: review/fix the V2 (domain entity vs ViewModel) and V1 (Index @model) findings in source.
- MCP `audit_conformance` tool added (batch-scans controllers/services vs ARCHITECTURE §4/AGENTS rules: C2 primary ctor, C3 access control, S1 IUnitOfWork-only, S2 no ApplicationDbContext, S3 try/catch on mutations, S4 AuditTrail on CUD). Also `trace_workflow` now truly recursive (depth-bounded, resolves member→Service/Repository file), `execute_sql` capped at 500 rows.
...
### Session Log
- 2026-08-04 — MCP view audit tooling — new `audit_views` tool batch-scans .cshtml against ARCHITECTURE.md §4.4 (V1 Index no @model, V2 ViewModel on Create/Edit, V3 modern-layout, V4 fetch error handling, V5 structural tag balance, V6 undeclared-var reads, V7 unused @model). V5/V6 refined to kill false positives: strip script/style/comments before counting tags; skip known globals + function params + property chains for V6. Baseline: 57 findings — 32 V2 (all Create/Edit pages use domain entities instead of ViewModels), 12 V7 (Index pages declare @model but never reference it — same root as V1), 9 V1 (Index declares @model), 4 V3 (Home/Index, MsapImport/Index, MonthlyClose/Index, Docs/Index lack modern-layout). V4/V5/V6 clean (all fetch() calls handle success/error; no broken tags; no ReferenceError reads).
