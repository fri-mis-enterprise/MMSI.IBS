    ...
    public class MaritimeReportController(IUnitOfWork unitOfWork) : Controller
    {
        ...
        public async Task<IActionResult> SalesSummary(int month, int year, CancellationToken ct)
        {
            ...
            foreach (var t in data)
            {
                ...
                if (netSalesCol.HasValue)
                {
                    SetDecimal(ws, row, netSalesCol.Value + 1, t.TotalNetRevenue);
                }
                ...
                if (tugboatCols.TryGetValue(tugName, out var tc))
                {
                    ...
                    {
                        SetDecimal(ws, row, tc.Income.Value + 1, t.TotalNetRevenue);
                    }

                    if (tc.Hours.HasValue)
                    {
                        SetDecimal(ws, row, tc.Hours.Value + 1, t.TotalHours);
                    }
                    ...
                    if (assistCol.HasValue)
                    {
                        ws.Cells[row, assistCol.Value + 1].Value = 1;
                    }

                    if (isTending)
                    {
                        if (tc.Tending.HasValue)
                        {
                            ws.Cells[row, tc.Tending.Value + 1].Value = 1;
                        }

                        var hoursCol = isForeign ? tc.TendingHoursForeign : tc.TendingHoursLocal;
                        if (hoursCol.HasValue)
                        {
                            SetDecimal(ws, row, hoursCol.Value + 1, t.TotalHours);
                        }
                    }
                }
