...
export function extractReferencedTypes(code: string) {
...
export async function findTypeDefinition(projectRoot: string, typeName: string) {
...
async function resolveMemberFile(projectRoot: string, member: string): Promise<string | null> {
  ...
  for (const pattern of patterns) {
    ...
    for (const file of files) {
      ...
      if (baseNames.has(base)) return file;
    }
  }
  return null;
}
 ...
 */
export async function traceWorkflowDeep(
  projectRoot: string,
  filePath: string,
  methodName: string,
  maxDepth: number = 3,
): Promise<{ file: string; method: string; calls: { member: string; method: string }[] }[]> {
  const fullPath = path.join(projectRoot, filePath);
  const body = await findMethodInFile(fullPath, methodName);
  if (!body) return [];

  const results: { file: string; method: string; calls: { member: string; method: string }[] }[] = [];
  const visited = new Set<string>();
  const queue: { file: string; method: string; depth: number }[] = [{ file: filePath, method: methodName, depth: 0 }];
...
export async function analyzeActionRelations(projectRoot: string, filePath: string, methodName: string) {
  const fullPath = path.join(projectRoot, filePath);
