...
function attributesAbove(content, matchIndex) {
...
function publicMethods(content) {
    const regex = /public\s+(?:async\s+)?([\w<>\[\]?,\.\s]+)\s+(\w+)\s*\(/g;
    const methods = [];
    let m;
    while ((m = regex.exec(content)) !== null) {
        if (/\b(?:class|interface|record|enum|struct)\b/.test(m[1]))
            continue;
...
const hasAccessAttr = (attrs) => /\[(RequireAccess|RequireAnyAccess|Authorize|AllowAnonymous)\b/.test(attrs);
const isActionResult = (rt) => /ActionResult|JsonResult|FileResult|IActionResult/.test(rt);
export async function auditControllers(projectRoot) {
    const findings = [];
    ...
    for (const file of files) {
        ...
        for (const m of publicMethods(content)) {
            if (!isActionResult(m.returnType))
                continue;
            if (classCovered || hasAccessAttr(m.attributes))
                continue;
            findings.push({
...
export async function auditServices(projectRoot) {
    const findings = [];
    const files = await glob(path.join(projectRoot, 'IBS.Services', '*.cs').replace(/\\/g, '/'));
    for (const file of files) {
        const content = fs.readFileSync(file, 'utf-8');
        const rel = path.relative(projectRoot, file).replace(/\\/g, '/');
        if (/^I[A-Z]/.test(path.basename(file)) || rel.endsWith('Middleware.cs'))
            continue;
        const classMatch = content.match(/class\s+(\w+)\s*\(([^)]*)\)/);
        if (!classMatch) {
