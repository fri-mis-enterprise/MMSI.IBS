...
function stripNonMarkup(content) {
    return content
...
export function auditViews(projectRoot, folder) {
    const findings = [];
    ...
    for (const file of files) {
        ...
        for (let i = 0; i < scriptBlocks.length; i++) {
            ...
            while ((r = readRe.exec(script)) !== null) {
                ...
                if (prev === '.' || prev === '$')
                    continue; // property access on another object (PageConfig.messages.error)
